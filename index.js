...
async function run() {
  try {
    ...
    const propertyName = core.getInput("property-name", {
      required: true,
    });
    const newValue = core.getInput("value", {
      required: true,
    });
    ...
    // Update the container's property to its new value
    ...
    containerDef[propertyName] = JSON.parse(newValue);
